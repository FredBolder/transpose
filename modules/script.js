...
async function ukuleleClicked(e) {
  if ((e.button === 0) && (ChordDiagrams.ukuleleFrets.length > 0)) {
    ...
    if (xMouse < p1) {
      ChordDiagrams.ukuleleVariation--;
      if (ChordDiagrams.ukuleleVariation < 0) {
        ChordDiagrams.ukuleleVariation = ChordDiagrams.ukuleleFrets.length - 1;
      }
      ChordDiagrams.drawUkulele(0, [], true, false);
    } else if ((xMouse >= p1) && (xMouse < p2)) {
      ChordDiagrams.drawUkulele(0, [], true, true);
    } else {
      ChordDiagrams.ukuleleVariation++;
      if (ChordDiagrams.ukuleleVariation >= ChordDiagrams.ukuleleFrets.length) {
        ChordDiagrams.ukuleleVariation = 0;
      }
      ChordDiagrams.drawUkulele(0, [], true, false);
    }
...
try {
  ...
  document.getElementById("guitarTuning").addEventListener("change", (e) => {
    ChordDiagrams.guitarTuning = document.getElementById("guitarTuning").value;
    if (Glob.settings.inputChordInfo.value.trim() !== "") {
      chordInfoClicked();
    }
  });
  ...
  document.getElementById("ukuleleLeftHanded").addEventListener("change", (e) => {
    ChordDiagrams.ukuleleLeftHanded = document.getElementById("ukuleleLeftHanded").checked;
    if (Glob.lastChord.length > 0) {
      ChordDiagrams.drawUkulele(0, [], true, false);
    }
  });
  ...
  document.getElementById("ukuleleTuning").addEventListener("change", (e) => {
    ChordDiagrams.ukuleleTuning = document.getElementById("ukuleleTuning").value;
    if (Glob.settings.inputChordInfo.value.trim() !== "") {
      chordInfoClicked();
    }
  });
  document.getElementById("btTuneUkulele").addEventListener("click", () => ChordDiagrams.tuneUkulele());
